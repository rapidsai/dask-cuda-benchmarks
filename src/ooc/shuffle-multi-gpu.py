...
def exchange_column_by_partition(comm, column, partition_offsets):
    size = comm.size
    N = len(partition_offsets)
    chunk_offsets = np.cumsum([0] + [N // size + ((N % size) > i) for i in range(size)])
    # TODO, not yet for string columns (need a level of indirection)
    sendcounts = np.diff(chunk_offsets)
    recvcounts = np.zeros(comm.size, dtype=sendcounts.dtype)
    comm.Alltoall(sendcounts, recvcounts)
    nrows = recvcounts.sum()
    # allocate column like input
    new_col = allocate_column_like(column, nrows)  # noqa:
    comm.Alltoallv(column, sendcounts, new_col, recvcounts)
    return new_col
